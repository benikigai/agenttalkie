...
The catalog observed during implementation contained 856 tools: 270 read tools, 458 actions requiring approval and 128 excluded account/administration/sharing/autonomous-agent tools. Catalog presence does not prove the credential permits every tool. Provider permission failures are shown as failures. No autonomous Ambiguous assistant is invoked. Up to four planning steps run per request; follow-up turns can continue longer workflows.
...
Real-provider verification passed on September 12: the live model selected `list_documents`, and the direct action flow created an unassigned task named `AgentTalkie direct-tools live verification`, ID `364ae743-59a3-471a-869c-bb802a0f9f8c`. A separate `get_task` call read it back. The isolated local verification thread was removed; the provider task remains. This verifies document discovery and task creation/readback, not every discovered tool.


### Production browser verification, September 12 at 23:30 UTC

Production revision 947bfec passed a complete direct `create_document` flow from the AgentTalkie browser: exact restricted-document preview, approval button, one provider write, separate `get_document`, and the resulting content displayed with its record ID. The document is `AgentTalkie live tools proof`, ID `525ccdea-eb22-4f44-a5f2-c4268ffeecc0`, at https://app.ambiguous.ai/docs/525ccdea-eb22-4f44-a5f2-c4268ffeecc0. It contains only neutral verification text. The human browser then opened that exact URL and displayed the matching title and full content, with a last-saved time of 4:30:12 PM Pacific. AgentTalkie and the Ambiguous document tabs were kept open for the demo.

Production also returned `auth_whoami`, `users_list`, `list_documents`, `get_task`, and the full content of the voice-created `Personal AI` document (`4b4343f1-a764-4d0e-bf52-b969fe1c83b8`). That document was independently opened in the human browser. The API caller is AgentTalkie in workspace `411b7a44-b117-461e-b9de-91e242701db2`; it is a separate identity from Benjamin Shyong. Its document list differs from the human document list. Reading the older VA document `e707e2cb-7fc1-4145-becd-166f4270d8bc` returned a provider error. The verification task is readable through MCP but its human-browser URL returned Task not found. The underlying cause of those record-specific access differences is unresolved; do not promise unrestricted access to every workspace record.

The full verification command passed, including 182 web tests. Later input-repair and redacted transport-error logging changes passed typecheck. Identity reads are explicitly permitted; account administration, permission changes and autonomous-assistant tools remain excluded. Reconnect any already-open voice session to load current tool instructions. Keep new UI work coordinated with Claude and preserve the live activity endpoint and approval controls.
