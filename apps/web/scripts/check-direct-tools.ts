...
const client={connection:{async listTools(){return {tools:[read,create,update]};},async callTool(input:{name:string;arguments:Record<string,unknown>}){if(input.name!=="get_task"){writes++;title=String(input.arguments.title);}return {content:[],structuredContent:{id:wrongReadback&&input.name==="get_task"?randomUUID():recordId,title}};}},async close(){}};
...
async function preview(name:string,args:Record<string,unknown>){const c=await request(`Use ${name} for ${JSON.stringify(args)}`);let turn=0;const result=await runDirectTools(c,c.request.question,async()=>++turn===1?{tools:[name,"get_task"],message:""}:{tool:name,argumentsJson:JSON.stringify(args),answer:""},client);await mutate(owner,sessionId,s=>{const r=s.requests.at(-1)!;r.state="completed";const {question,...identity}=c.request;r.result={...identity,...result};});return c;}
try{
 await preview("create_task",{title:"Exact preview"});assert.equal(writes,0);
 const approved=await request("approve workspace action");
 const attempts=await Promise.allSettled([approveDirectTool(approved,()=>client),approveDirectTool(approved,()=>client)]);
 assert.equal(attempts.filter(a=>a.status==="fulfilled").length,1);assert.equal(writes,1);
 const replay=await request("approve workspace action");await approveDirectTool(replay,()=>client);assert.equal(writes,1);
 await preview("update_task",{id:recordId,title:"Requested edit"});title="Changed by another editor";
 await assert.rejects(approveDirectTool(await request("approve workspace action"),()=>client),/record changed/);assert.equal(writes,1);
 await preview("create_task",{title:"Unknown readback"});wrongReadback=true;
 await assert.rejects(approveDirectTool(await request("approve workspace action"),()=>client),/could not be verified/);assert.equal(writes,2);
 await assert.rejects(approveDirectTool(await request("approve workspace action"),()=>client),/not be retried/);assert.equal(writes,2);
 ...
 await assert.rejects(approveDirectTool(stale,()=>client),/superseded/);assert.equal(writes,2);
