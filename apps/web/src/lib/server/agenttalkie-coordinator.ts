import {latestArtifact} from "./agenttalkie-artifacts";
...
export const Intent=z.object({action:z.enum(["orient","list_tasks","research","investigate","review","build","edit","draft_document","save_document","workspace_tool","approve_tool","clarify"]),question:z.string().min(1).max(4000),correction:z.boolean()}).strict();
const intentSchema={type:"object",properties:{action:{type:"string",enum:["orient","list_tasks","research","investigate","review","build","edit","draft_document","save_document","workspace_tool","approve_tool","clarify"]},question:{type:"string"},correction:{type:"boolean"}},required:["action","question","correction"],additionalProperties:false};
...
${prepared.draft.content}`;
  ...
  } else if(selected.action==="build"||selected.action==="edit"){
   const runner=await sql()`SELECT id FROM agenttalkie_runners WHERE heartbeat>now()-interval '45 seconds' AND build_version=1 LIMIT 1`;
   if(!runner.length)throw new AgentTalkieError(503,"BUILD_RUNNER_OFFLINE","The isolated build runner is offline. No demo files were changed.");
   let context:Record<string,unknown>={};
   if(selected.action==="edit"){
    const parent=await latestArtifact(owner,sessionId);
    if(!parent)throw new AgentTalkieError(409,"BUILD_ARTIFACT_REQUIRED","First ask Codex to build the playable app. Claude edits that exact saved artifact.");
    context={parentArtifactId:parent.id,artifactHtml:parent.html,contentHash:parent.content_hash};
   }
   ...
   await recordEvent(owner,sessionId,request,"ori",selected.action==="build"?"Codex demo build queued":"Claude artifact edit queued","pending",{kind:"worker_queued"});
   return;
