...
const STARTER_PROMPTS = [
  { label: "Build a playable game", tool: "Codex CLI", send: "Have Codex build a black-and-white tic-tac-toe game in the demo workspace: two human players, alternating turns, win and draw detection, and a reset button. Write a self-contained index.html with accessible cell buttons and a status message." },
  { label: "Create the demo task", tool: "Ambiguous", send: "Create an Ambiguous task titled AgentTalkie tic-tac-toe demo. Description: Build a playable two-player game with Codex, then have Claude edit X to green and O to red. Preview the exact task creation action." },
  ...
  { label: "Browse my Ambiguous workspace", tool: "Workspace", send: "List the documents, tasks and sheets in my Ambiguous workspace. Show the real titles and record IDs." },
        ...
        {view === "history" ? <>
          ...
          {!currentRequest ? <div className="at-intro"><h2>Start here</h2><p>Ask about this agent's work, then narrow the question as you go. Corrections keep the earlier answer in history.</p>
            ...
            {answer && <article className="at-response"><div className="at-response-header"><span className="at-agent-monogram" style={{ width: 28, height: 28, fontSize: 16 }}>{target.agentName.slice(0, 1)}</span><strong>{target.agentName}</strong><span className="at-result-caption">Finding received</span></div>
              ...
              <div className="at-actions">{answer.evidence.some(e=>e.kind==="checkpoint"&&e.reference.startsWith("AgentTalkie workspace action ")) && <button className="at-button at-button-primary" disabled={!available || submitting} onClick={()=>void workspace.sendQuestion("approve workspace action")}>Approve workspace action</button>}{answer.evidence.some(e=>e.kind==="worker_reply"&&e.reference.startsWith("Ori codex job ")) && !answer.evidence.some(e=>e.reference.includes("/live/artifact/")) && <button className="at-button at-button-primary" disabled={!available || submitting} onClick={()=>void workspace.sendQuestion("Have Claude Code independently review this exact Codex artifact against the same repository file.")}>Review with Claude Code</button>}{answer.evidence.some(e=>e.kind==="checkpoint"&&e.reference.startsWith("AgentTalkie document draft ")) && <button className="at-button at-button-primary" disabled={!available || submitting} onClick={()=>void workspace.sendQuestion("save this document")}>Save this document to Ambiguous</button>}<button className="at-button" disabled={!available || submitting || preparing} onClick={() => { setCopiedKey(null); void workspace.prepare(); }}>{preparing ? "Preparing…" : "Prepare follow-up"}<Icon name="arrow" size={14} /></button><button className="at-button" onClick={() => setView("history")}>View history</button></div>
            </article>}
